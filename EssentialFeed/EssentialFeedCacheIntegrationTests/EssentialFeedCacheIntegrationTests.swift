...
//
//  Created by Fer Benavides Rodriguez on 22/07/25.
//
...
final class EssentialFeedCacheIntegrationTests: XCTestCase {
    ...
    func test_loadFeed_deliversItemsSavedOnASeparateInstance() {
        ...
        expect(feedLoaderPerformLoad, toLoad: feed)
    }
    ...
    func test_validateFeedCache_doesNotDeleteRecentlySavedFeed() {
        ...
        save(feed, with: feedLoaderToPerformFirstSave)
        validateCache(with: feedLoaderToPerformValidation)
        
        expect(feedLoaderToPerformFirstSave, toLoad: feed)
    }
    ...
    func test_loadImageData_deliversSavedDataOnASeparateInstance() {
        ...
        let imageLoaderToPerfromLoad = makeImageLoader()
        let feedLoader = makeFeedLoader()
        ...
        expect(imageLoaderToPerfromLoad, toLoad: dataToSave, for: image.url)
    }
    ...
    private func expect(_ sut: LocalFeedLoader, toLoad expectedFeedImage: [FeedImage], file: StaticString = #file, line: UInt = #line) {
        let exp = expectation(description: "Wait for load completion")
        sut.load { result in
            switch result {
            case let .success(loadedFeed):
                XCTAssertEqual(loadedFeed, expectedFeedImage, file: file, line: line)
    ...
    private func save(_ data: Data, for url: URL, with loader: LocalFeedImageDataLoader, file: StaticString = #file, line: UInt = #line) {
        let saveExp = expectation(description: "Wait for save comletion")
