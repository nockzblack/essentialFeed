...
final class FeedAcceptanceTests: XCTestCase {
    ...
    func test_onLaunch_displaysEmotyFeedWhenCustomerHasNoconnectivityAndNoCache() {
    ...
    func test_onFeedImageSelection_displaysComments() throws {
        if #available(iOS 17.0, *) {
            throw XCTSkip("This fails on iOS 17+. Needs refactor")
        }
    ...
    private func launch(
        ...
        let sut = SceneDelegate(httpClient: httpClient, store: store, scheduler: .inmmediateOnMainQueue)
    ...
    private func enterBackground(with store: InMemoryFeedStore) {
        let sut = SceneDelegate(httpClient: HTTPClientStub.offline, store: store, scheduler: .inmmediateOnMainQueue)
    ...
    private func makeData(for url: URL) -> Data {
        ...
        case "/essential-feed/v/image/2AB2AE66-A4B7-4A16-B374-51BBAC8DB086/comments":
